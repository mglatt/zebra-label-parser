...
3. **Extract** — If an Anthropic API key is configured, Claude Vision identifies the shipping label bounding box and crops it. Without a key, a heuristic fallback crops the upper-left region of letter-size pages (where carriers typically place the label).
   The division of labor is deliberate: the Vision prompt owns the *semantics* (what belongs to the label — rotated address blocks are included, return-authorization slips and sidebar headings excluded), while `app/services/crop_geometry.py` owns *geometric safety*. Given the proposed box it may only **grow** an edge that slices through ink (recovering clipped barcodes or missed address columns), **shed** content separated from the label by a clean whitespace gap, and apply one final trim-to-content + margin. It never cuts through ink. All of its lengths are expressed in inches and scaled by the working resolution, so the same document crops identically at any DPI, and the expected label proportions come from the configured label size.
